...
function ExchangeItem({ navigation, route }) {
    ...
    const [nameJarTo, setnameJarTo] = useState("");
    ...
    useEffect(() => {
        if (route.params != undefined) {
            const { typeXL, name, idJar, availableBalances, precent, totalIncome, totalSpending } = route.params;
            if (typeXL == 1) {
                setcolorThuNhap("#F9B79C");
                setcolorChiTieu("#E6E6FA");
                setcolorChuyenTien("#E6E6FA");
                ...
                setNameJar(name);
                setisJar(idJar);
                setavailableBalancesI(availableBalances);
                SetprecentJar(precent);
            ...
            <ScrollView style={styles.scrollview}>
                ...
                }
                {
                    ...
                    <View style={{ marginLeft: 20, marginTop: 20 }}>
                        <TouchableOpacity onPress={
                            () => {
                                navigation.navigate('CameraPicItem');
                            }
                ...
                }
                {
                    ...
                    <View>
                        ...
                        <View style={styles.containerJar}>
                            <SelectDropdown
                                data={dataJarTo}
                                defaultButtonText={nameJarTo}
                                buttonTextStyle={{ fontSize: 16, }}
                                onSelect={(selectedItem, index) => {
                                    setnameJarTo(selectedItem);
                                ...
                                renderCustomizedButtonChild={value => {
                                    return (
                                        <View style={{ flexDirection: 'row', marginRight: 8, alignItems: 'center', flex: 1 }}>
                                            ...
                                            <View>
                                                <Text style={{ fontSize: 22, marginLeft: 20 }}>{nameJarTo}</Text>
